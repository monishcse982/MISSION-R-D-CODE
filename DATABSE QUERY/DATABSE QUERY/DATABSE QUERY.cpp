// DATABSE QUERY.cpp : Defines the entry point for the console application.
//
...
{
	FILE *student_db = fopen("D:\\MISSION R&D\\C ONLINE COURSE\\DATABSE QUERY\\DATABSE QUERY\\STUDENTS DATABSE.txt","r+");
	STUDENTS *details = (STUDENTS*)malloc(1*sizeof(STUDENTS));
	int index=0;
	char ch = fgetc(student_db);
	while (ch!=EOF)
	{
		details = (STUDENTS*)realloc(details,(sizeof(STUDENTS))*(index+2));
		fscanf(student_db,"%d",&details[index].roll_no);
		fgetc(student_db);
		...
		while(ch!=':' && ch!='\n')
		{
			details[index].name[string_index] = ch;
			ch = fgetc(student_db);
			string_index++;
		}
		details[index].name[string_index] = '\0';
		string_index = 0;
		fscanf(student_db,"%d",&details[index].marks);
		fgetc(student_db);
		ch = fgetc(student_db);
		while(ch!='\n' && ch!=EOF)
		{
			details[index].address[string_index] = ch;
			ch = fgetc(student_db);
			string_index++;
		}
		details[index].address[string_index] = '\0';
		printf("--------------------------\n");
		printf("ROLL NO : %d\n",details[index].roll_no);
		printf("NAME : %s\n",details[index].name);
		printf("MARKS : %d\n",details[index].marks);
