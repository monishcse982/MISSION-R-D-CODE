...
{
	char buffer[16];
	struct META_RECORD *node=(META_RECORD *)malloc(sizeof(META_RECORD ));
	...
	fwrite(node,sizeof(META_RECORD),1,fp);
	return index;
}
...
META_RECORD* search(char *buffer,FILE *fp,int total,char *bitvector)
{
	META_RECORD *node=(META_RECORD *)malloc(sizeof(META_RECORD ));
	for(int i=5;i<=total;i++)
	{
		if(bitvector[i]=='1')
		{
			fseek(fp,i*32,SEEK_SET);		
			fread(node,sizeof(struct META_RECORD),1,fp);
...
void deleteD(FILE *fp,int index,char *bitvector)
{
	...
	scanf("%s",&buffer);
	META_RECORD *x=search(buffer,fp,index,bitvector);
	if(x == NULL)
	...
	else
	{
			...
			fseek(fp,(x->start_pos)
*32,SEEK_SET);//going to the particular META_RECORD
			fwrite(buffer,sizeof(buffer),1,fp);
...
void file_search(FILE *fp,int index,char *bitvector)
{
			...
			scanf("%s",buffer);
			META_RECORD *x=search
(buffer,fp,index,bitvector);
			if(x == NULL)
				printf("name not found");
			else
			{
...
void file_list(FILE *fp,char *bitvector,int index)
{
	...
	for(int i=5;i<=index;i++)
	{
		...
		{
			fseek(fp,i*32,SEEK_SET);
			fread(node,sizeof(META_RECORD),1,fp);
			printf(" %s\n",node->filename);
		}
	}
}
...
int file_dump(FILE *fp,int index,char *bitvector)
{
	...
	scanf("%s",buffer);
	META_RECORD *node=search(buffer,fp,index,bitvector);
	if(node== NULL)
	{
		printf("not found");
		return index;
